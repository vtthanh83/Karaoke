    ...
    public partial class frmReceipt : Form
    {
        ...
        private void numReduce_ValueChanged(object sender, EventArgs e)
        {
            if (iCurrentReceiptID > -1)
            {
                ...
                if (currentReceipt.Trangthai > 1)
                    return;
                currentReceipt.Tanggio = Convert.ToInt32(numReduce.Value);
                //update DB
                bool res = new DataAccess().updateHoadonxuat(currentReceipt);
                if (!res)
                {
        ...
        private void gridViewBillProduct_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                if ((currentReceipt.Trangthai == (int)ReceiptStatus.Open) || (Program.userLevel == Level.Admin))
                {
                ...
                {
                    MessageBox.Show(this, "Hóa đơn không được thay đổi. Liên hệ quản lý để thay đổi", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            timer1.Stop();
            fa.ShowDialog();
            if (fa.closeReceipt.Count > 0) //close some receipt
            {
                
                for (i = 0; i < fa.closeReceipt.Count; i++)
                {
                    Hoadonxuat cr = new DataAccess().getHoadonxuatByID(fa.closeReceipt.ElementAt(i));
                    if (cr != null)
                    {
                        switch (cr.Trangthai)
                        {
                            case (int)ReceiptStatus.Paid:
                                cr.Trangthai = (int)ReceiptStatus.Closing;
                                break;
                            case (int)ReceiptStatus.Printed:
                                cr.Trangthai = (int)ReceiptStatus.ClosingNotPaid;
                                cr.Suco = 1;
                                cr.Ghichu = cr.Ghichu + " - Đóng hóa đơn khi chưa xác nhận thu tiền";
                                break;
                            case (int)ReceiptStatus.Open:
                                cr.Trangthai = (int)ReceiptStatus.ClosingNotPrint;
                                cr.Suco = 1;
                                cr.Ghichu = cr.Ghichu + " - Đóng hóa đơn khi chưa in hóa đon";
                                break;
                            default:
                                break;
                        }
                        //update Receipt status
                        if (new DataAccess().updateHoadonxuat(cr))
                        {
                            //update Phong status
                            Phong room = new DataAccess().getPhongByID(cr.IDPhong);
                            if (room != null)
                            {
                                room.Trangthai = false;
                                if (new DataAccess().updatePhong(room) != true)
                                {
                                    MessageBox.Show("Không thể đóng phòng " + room.TenPhong, "Lỗi dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                                //tat CB
                                ContactAction(0xfb, room.Congtac);
                            }
                        }
                        else
                            MessageBox.Show("Không thể đóng hóa đơn " + cr.TenHoadon, "Lỗi dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            if (fa.continueReceipt.Count > 0) //open new some receipt
            {
              
                for (i = 0; i < fa.continueReceipt.Count; i++)
                {
                    Hoadonxuat cr = new DataAccess().getHoadonxuatByID(fa.continueReceipt.ElementAt(i));
                    if (cr != null)
                    {
                        switch (cr.Trangthai)
                        {
                        ...
                        if (new DataAccess().updateHoadonxuat(cr))
                        {
                            ...
                            if (room != null)
                            {
                                ...
                                else //open new receipt
                                {
                                    iCurrentReceiptID = OpenNewReceipt(room.IDPhong);
                                    if (iCurrentReceiptID < 0)
                                        MessageBox.Show("Không thể mở hóa đơn với phòng " + cr.TenHoadon, "Lỗi dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                            }
                        }
                        else
        ...
        */
        private void Start(int roomID)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (Convert.ToBoolean(ds.Tables[0].Rows[0]["Trangthai"]) == true)
                {
                    ...
                    currentReceipt.GioKT = Convert.ToDateTime(lastBill.Tables[0].Rows[0]["GioKT"]); ;
                    ...
                    currentReceipt.Tanggio = Convert.ToInt32(lastBill.Tables[0].Rows[0]["Tanggio"]);
                    currentReceipt.Suco = Convert.ToInt32(lastBill.Tables[0].Rows[0]["Suco"]);
                    currentReceipt.Trangthai = Convert.ToInt32(lastBill.Tables[0].Rows[0]["Trangthai"]);
                    if (currentReceipt.Trangthai != (int)ReceiptStatus.Paid)
                    {
                        if (MessageBox.Show("Hóa đơn chưa xác nhận thu tiền. Bạn có muốn tiếp tục đóng phòng " + Convert.ToString(ds.Tables[0].Rows[0]["TenPhong"]) + "?", "Thông báo đóng phòng", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                            return;
                    ...
                    if (res)
                    {
                        ...
                        updateRoom.Congtac = Convert.ToInt32(ds.Tables[0].Rows[0]["Congtac"]);
        ...
        private int getTotalValueOfBill(int IDHoadon)
        {
            ...
            int KM = Convert.ToInt32(dsBill.Tables[0].Rows[0]["Tanggio"]);
            ...
            int totalBill = 0;
            totalBill = roomMoney + productMoney + phuthu+KM;
        ...
        private bool updateBillDisplay(int IDHoadon, bool timeupdate)
        {
            ...
            txtProductMoney.Text = productMoney.ToString("###,###,###,##0");
            int totalBill = 0;
            totalBill = roomMoney + productMoney + Convert.ToInt32(numExtra.Value) + Convert.ToInt32(numReduce.Value);
