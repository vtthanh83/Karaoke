...
namespace BKIT.Model.DataService
{
    public class HoadonxuatService : IHoadonxuat
    {
        public int insertHoadonxuat(BKIT.Entities.Hoadonxuat objHoadonxuat)
        {
            ...
            string sqlCommand = "INSERT INTO Hoadonxuat(IDHoadonXuat,Ngayxuat,IDNhanvien,IDPhong,GioBD,GioKT,Tratruoc,Phuthu,Thue,Giam,Ghichu,IDGiaLoaiphong,Trangthai, Tanggio, IDKhachhang, Nhacnho) " +
            "VALUES (@idhoadonxuat,@ngayxuat,@idnhanvien,@idphong,@giobd,@giokt,@tratruoc,@phuthu,@thue,@giam,@ghichu,@idgialoaiphong,@trangthai,@tanggio,@idkhachhang,@nhacnho);";
        ...
        {
            Database db = DatabaseFactory.CreateDatabase();
            string sqlCommand = "UPDATE Hoadonxuat SET Ngayxuat = @ngayxuat, IDNhanvien = @idnhanvien, IDPhong = @idphong, GioBD = @giobd, GioKT = @giokt, Tratruoc = @tratruoc, Phuthu = @phuthu, Thue = @thue, Giam = @giam, Ghichu = @ghichu, IDGiaLoaiphong = @idgialoaiphong, Trangthai = @trangthai, Tanggio = @tanggio, IDKhachhang = @idkhachhang, Nhacnho = @nhacnho  " +
            ...
            try
            {
                ...
                db.AddInParameter(dbCommand, "tanggio", DbType.Int32, objHoadonxuat.Tanggio);
                db.AddInParameter(dbCommand, "idkhachhang", DbType.Int32, objHoadonxuat.IDKhachhang);
                db.AddInParameter(dbCommand, "idkhachhang", DbType.Boolean, objHoadonxuat.Nhacnho);
                db.AddInParameter(dbCommand, "idhoadonxuat", DbType.Int32, objHoadonxuat.IDHoadonXuat);
                db.ExecuteNonQuery(dbCommand);
                dbCommand.Connection.Close();
                return true;
        ...
        {
            Database db = DatabaseFactory.CreateDatabase();
            string sqlCommand = "SELECT IDHoadonXuat, Ngayxuat, IDNhanvien, GioBD, GioKT, Tratruoc, Phuthu, Thue, IIf([Trangthai]=0,'Mở','Đóng') AS Tinhtrang, IDKhachhang, 1 AS [DeleteHDXuat] FROM Hoadonxuat WHERE IDPhong<0";
            try
            {
                DbCommand dbCommand = db.GetSqlStringCommand(sqlCommand);
                DataSet ds = db.ExecuteDataSet(dbCommand);
