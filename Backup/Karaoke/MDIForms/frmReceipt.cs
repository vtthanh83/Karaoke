    ...
    public partial class frmReceipt : Form
    {
        ...
        private void AddSP(int num)
        {
            ...
            if (gridViewSanPham.FocusedRowHandle >= 0)
            {
                ...
                //}
                //else
                //    MessageBox.Show("Sản phẩm " + gridViewSanPham.GetRowCellValue(gridViewSanPham.FocusedRowHandle, "TenSanPham") + " chưa có giá bán! \nVui lòng nhập giá bán bên cửa sổ quản trị", "Thông báo giá bán", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //search for warining to close room
            DataSet ds = new DataAccess().getAllOpenningHoadonxuatWithDeposit();
            if (ds == null) return;
        ...
        {
            if (displayCurrentRoomReceipt(iReceiptIndex + 1))
                iReceiptIndex++;
        }
        ...
        {
            displayFirstRoomReceipt();
        }
        private void dateReceipt_ValueChanged(object sender, EventArgs e)
        {
            if (checkDateReceipt.Checked)
                displayLastRoomReceipt();
        }
        ...
        //    {
        //        int i;
        //        DataSetDetailBill dsBill = new DataSetDetailBill();
        ...
        //        dr["Ngayxuat"] = dateReceipt.Value.ToString("dd/MM/yyyy");
        ...
        //        dr["GioBD"] = Convert.ToDateTime(dsHD.Tables[0].Rows[0]["GioKT"]).ToShortTimeString();
        ...
        private void btnJoinRoom_Click(object sender, EventArgs e)
        {
            if (iCurrentReceiptID >= 0)
            {
                ...
                for (i = 0; i < dsSP.Tables[0].Rows.Count; i++)
                {
                    if (Convert.ToBoolean(dsSP.Tables[0].Rows[i]["Kho"]) == false)
                    {
                        DataRow dr1 = dsBill.Tables["RDatadetail"].NewRow();
                        dr1["STT"] = (i + 1).ToString();
                        dr1["TenSanPham"] = dsSP.Tables[0].Rows[i]["TenSanPham"].ToString();
                    ...
                    if (frmView.ShowDialog() == DialogResult.Yes)
                    {
                        for (i = 0; i < dsSP.Tables[0].Rows.Count; i++)
                        {
                            if (Convert.ToBoolean(dsSP.Tables[0].Rows[i]["Kho"]) == false)
                            {
                                ...
                                obj.Bep = Convert.ToBoolean(dsSP.Tables[0].Rows[i]["Bep"]);
        ...
        private void AddSPBD(int IDReceipt, int IDLoaiSP,int IDSanPham, string TenSP,int num)
        {
            ...
            //check if the parameters are legal
                ...
                if (IDReceipt >= 0)
                {
                    ...
                    try{
                        obj.IDGiaxuat = Convert.ToInt32(dsGia.Tables[0].Rows[0]["IDGiaXuatSP"]);
                    }
                    catch
                    {
